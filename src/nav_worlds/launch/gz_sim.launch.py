...
def launch_gz(context, *args, **kwargs):
    ...
    # Use shlex.split to construct an argument list without running inside /bin/sh (shell=False).
    # Running with shell=False ensures ROS 2 launch shutdown signals (SIGINT/SIGTERM) propagate
    # directly to the Ruby wrapper process and trigger its internal traps to terminate both
    # gz sim server and gz sim gui cleanly, preventing lingering GUI windows.
    cmd = shlex.split(f"{exec} {exec_args} --force-version {gz_version}")
